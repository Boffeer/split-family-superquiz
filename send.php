...
if (empty($errors)) {
	...
	if ($user_name != '-') {
		$message .=
			"<h3>ФИО прошедшего квиз</h3> " . $user_name;
	}
	if ($user_tel != '-') {
		$message .=
			"<br><br><h3>Телефон прошедшего квиз</h3> " . $user_tel;
	}
	if ($user_email != '-') {
		$message .=
			"<br><br><h3>Email прошедшего квиз</h3> " . $user_email;
	}
	if ($spouse_email != '-') {
		$message .=
			"<br><br><h3>ФИО второго супруга</h3> " . $spouse_name;
	}
	if ($spouse_email != '-') {
		$message .=
			"<br><br><h3>Телефон второго супруга</h3> " . $spouse_tel;
	}
	if ($spouse_email != '-') {
		$message .=
			"<br><br><h3>Email второго супруга</h3> " . $spouse_email;
	}
	if ($question_divorce__plan != '-') {
		$message .=
			"<br><br><h3>Планируют развод?</h3> " . $question_divorce__plan;
	}
	if ($question_divorce__relations != '-') {
		$message .=
			"<br><br><h3>Фактически отношения были прекращены?</h3> " . $question_divorce__relations;
	}
	if ($question_divorce__divorce_date != '-') {
		$message .=
			"<br><br><h3>Дата расторжения брака</h3> " . $question_divorce__divorce_date;
	}
	if ($question_divorce__together != '-') {
		$message .=
			"<br><br><h3>Живут совместно?</h3> " . $question_divorce__together;
	}
	if ($question_divorce__date_no_relations != '-') {
		$message .=
			"<br><br><h3>Не ведет общее хозяйство с супругом с</h3> " . $question_divorce__date_no_relations;
	}
	if ($question_children__have != '-') {
		$message .=
			"<br><br><h3>Есть совместные дети?</h3> " . $question_children__have;
	}
	if ($question_children__age != '-') {
		$message .=
			"<br><br><h3>Имена и возраст детей</h3> " . $question_children__age;
	}
	if ($question_children__live_with_you != '-') {
		$message .=
			"<br><br><h3>Дети проживают с вами?</h3> " . $question_children__live_with_you;
	}
	if ($question_children__dispute_livingplace != '-') {
		$message .=
			"<br><br><h3>Есть ли спор относительно места жительства детей?</h3> " . $question_children__dispute_livingplace;
	}
	if ($question_children__livingplace_list != '-') {
		$message .=
			"<br><br><h3>Где должны проживать дети?</h3> " . $question_children__livingplace_list;
	}
	if ($question_children__dispute_communication != '-') {
		$message .=
			"<br><br><h3>Есть ли спор относительно общения с детьми?</h3> " . $question_children__dispute_communication;
	}
	if ($question_children__communication_schedule != '-') {
		$message .=
			"<br><br><h3>Приемлемый график общения детей (с учетом занятости детей, удаленности проживания) с родителем, который проживает отдельно от детей</h3> " . $question_children__communication_schedule;
	}
	if ($question_alimoney__dispute != '-') {
		$message .=
			"<br><br><h3>Есть ли спор по алиментным обязательствам?</h3> " . $question_alimoney__dispute;
	}
	if ($question_alimoney__recovered != '-') {
		$message .=
			"<br><br><h3>Алименты уже взысканы?</h3> " . $question_alimoney__recovered;
	}
	if ($question_alimoney__amount != '-') {
		$message .=
			"<br><br><h3>Какую сумму вы получаете (платите) или должны получать (платить)</h3> " . $question_alimoney__amount;
	}
	if ($question_alimoney__payer_official_income != '-') {
		$message .=
			"<br><br><h3>Плательщик алиментов имеет официальный доход?</h3> " . $question_alimoney__payer_official_income;
	}
	if ($question_alimoney__payer_income != '-') {
		$message .=
			"<br><br><h3>Какая общая сумма его дохода?</h3> " . $question_alimoney__payer_income;
	}
	if ($question_alimoney__payer_unofficial != '-') {
		$message .=
			"<br><br><h3>Плательщик алиментов работает без оформления?</h3> " . $question_alimoney__payer_unofficial;
	}
	if ($question_alimoney__payer_unemployed != '-') {
		$message .=
			"<br><br><h3>Плательщик алиментов безработный?</h3> " . $question_alimoney__payer_unemployed;
	}
	if ($question_alimoney__payer_businessman != '-') {
		$message .=
			"<br><br><h3>Плательщик алиментов предприниматель?</h3> " . $question_alimoney__payer_businessman;
	}
	if ($question_alimoney__payer_credit != '-') {
		$message .=
			"<br><br><h3>Сумма задолженности?</h3> " . $question_alimoney__payer_credit;
	}
	if ($question_alimoney__dispute_result != '-') {
		$message .=
			"<br><br><h3>Опишите как вы считаете, как должен быть разрешен вопрос по алиментам (сумма, сроки, единовременные выплаты и т.д.)</h3> " . $question_alimoney__dispute_result;
	}
	if ($question_alimoney__payer_children_amount != '-') {
		$message .=
			"<br><br><h3>Сколько детей у плательщика алиментов от иных отношений?</h3> " . $question_alimoney__payer_children_amount;
	}
	if ($question_alimoney__your_additional_expenses != '-') {
		$message .=
			"<br><br><h3>Доп расходы на детей помимо обычного содержания?</h3> " . $question_alimoney__your_additional_expenses;
	}
	if ($question_alimoney__your_expenses_education != '-') {
		$message .=
			"<br><br><h3>Доп расходы на образование?</h3> " . $question_alimoney__your_expenses_education;
	}
	if ($question_alimoney__your_expenses_rehabilitation != '-') {
		$message .=
			"<br><br><h3>Доп расходы на лечение (реабилитацию)?</h3> " . $question_alimoney__your_expenses_rehabilitation;
	}
	if ($question_alimoney__your_expenses_home != '-') {
		$message .=
			"<br><br><h3>Доп расходы на жилье?</h3> " . $question_alimoney__your_expenses_home;
	}
	if ($question_alimoney__your_expenses_children_about != '-') {
		$message .=
			"<br><br><h3>Иные дополнительные расходы на детей?</h3> " . $question_alimoney__your_expenses_children_about;
	}
	if ($question_alimoney__your_expenses_children != '-') {
		$message .=
			"<br><br><h3>Сколько иных дополнительных расходов на детей?</h3> " . $question_alimoney__your_expenses_children;
	}
	if ($question_alimoney__maintenanse != '-') {
		$message .=
			"<br><br><h3>Претендует ли вы на ваше содержание от второго супруга?</h3> " . $question_alimoney__maintenanse;
	}
	if ($question_alimoney__little_children != '-') {
		$message .=
			"<br><br><h3>Беременны или находитесь в отпуске по уходу за ребенком до трех лет?</h3> " . $question_alimoney__little_children;
	}
	if ($question_alimoney__maintenanse_disabled != '-') {
		$message .=
			"<br><br><h3>Осуществляете уход за общим ребенком инвалидом?</h3> " . $question_alimoney__maintenanse_disabled;
	}
	if ($question_alimoney__you_disabled != '-') {
		$message .=
			"<br><br><h3>Стали нетрудоспособным до расторжения брака или в течение года с момента расторжения брака?</h3> " . $question_alimoney__you_disabled;
	}
	if ($question_alimoney__you_need_help != '-') {
		$message .=
			"<br><br><h3>Нуждающийся бывший супруг, достигший пенсионного возраста не позднее, чем через 5 лет с момента расторжения брака, который длился более 10 лет?</h3> " . $question_alimoney__you_need_help;
	}
	if ($question_alimoney__maintenanse_amount != '-') {
		$message .=
			"<br><br><h3>Какую сумму вы считаете приемлемой на свое содержание?</h3> " . $question_alimoney__maintenanse_amount;
	}
	if ($question_money__jointly_property != '-') {
		$message .=
			"<br><br><h3>У вас существует спор относительно совместно нажитого имущества?</h3> " . $question_money__jointly_property;
	}
	if ($question_money__disput_moments != '-') {
		$message .=
			"<br><br><h3>Спорные моменты существуют у вас при расторжении брака?</h3> " . $question_money__disput_moments;
	}
	if ($question_money__marriage_agreement != '-') {
		$message .=
			"<br><br><h3>Был ли заключен брачный договор до или во время брака?</h3> " . $question_money__marriage_agreement;
	}
	if ($question_money__jointly_property_list != '-') {
		$message .=
			"<br><br><h3>Наименования совместно нажитого имущества</h3> " . $question_money__jointly_property_list;
	}
	if ($question_money__jointly_property_cost != '-') {
		$message .=
			"<br><br><h3>Примерная рыночная стоимость совместно нажитого имущества</h3> " . $question_money__jointly_property_cost;
	}
	if ($question_money__jointly_money != '-') {
		$message .=
			"<br><br><h3>Совместно нажитых денежных средств было вложено в приобретение имущества</h3> " . $question_money__jointly_money;
	}
	if ($question_money__maternity_capital != '-') {
		$message .=
			"<br><br><h3>Средств мат. капитала было вложено в приобретение имущества</h3> " . $question_money__maternity_capital;
	}
	if ($question_money__wifes_personal != '-') {
		$message .=
			"<br><br><h3>Личных средств жены от продажи личного имущества</h3> " . $question_money__wifes_personal;
	}
	if ($question_money__mans_personal != '-') {
		$message .=
			"<br><br><h3>Личных средств мужа от продажи личного имущества</h3> " . $question_money__mans_personal;
	}
	if ($question_money__credit_money != '-') {
		$message .=
			"<br><br><h3>Кредитных средств из все еще не погашенных кредитов</h3> " . $question_money__credit_money;
	}
	if ($question_money__repaired_estate != '-') {
		$message .=
			"<br><br><h3>Есть ли такое недвижимое имущество, принадлежащее хоть одному супругу, которое ремонтировалось или достраивалось в браке</h3> " . $question_money__repaired_estate;
	}
	if ($question_money__repaired_estate_cost != '-') {
		$message .=
			"<br><br><h3>Стоимость до ремонта</h3> " . $question_money__repaired_estate_cost;
	}
	if ($question_money__repaired_estate_additional_cost != '-') {
		$message .=
			"<br><br><h3>Стоимость после ремонта</h3> " . $question_money__repaired_estate_additional_cost;
	}
	if ($question_money__repaired_estate_jointly != '-') {
		$message .=
			"<br><br><h3>Совместно нажитых денежных средств было вложено в ремонт имущества</h3> " . $question_money__repaired_estate_jointly;
	}
	if ($question_money__repaired_estate_maternity != '-') {
		$message .=
			"<br><br><h3>Средств мат. капитала было вложено в ремонт имущества</h3> " . $question_money__repaired_estate_maternity;
	}
	if ($question_money__repaired_estate_wifes_personal != '-') {
		$message .=
			"<br><br><h3>Личных средств жены от продажи личного имущества</h3> " . $question_money__repaired_estate_wifes_personal;
	}
	if ($question_money__repaired_estate_mans_personal != '-') {
		$message .=
			"<br><br><h3>Личных средств мужа от продажи личного имущества?</h3> " . $question_money__repaired_estate_mans_personal;
	}
	if ($question_money__repaired_estate_credit != '-') {
		$message .=
			"<br><br><h3>Кредитных средств из все еще не погашенных кредитов?</h3> " . $question_money__repaired_estate_credit;
	}
	if ($question_credits__obligations != '-') {
		$message .=
			"<br><br><h3>Есть ли совместно нажитые обязательства — долги, кредиты, ипотека</h3> " . $question_credits__obligations;
	}
	if ($question_credits__obligations_target != '-') {
		$message .=
			"<br><br><h3>Наименования кредитов и на какие цели выдавался</h3> " . $question_credits__obligations_target;
	}
	if ($question_credits__obligations_rest != '-') {
		$message .=
			"<br><br><h3>Остаток задолженности на момент прекращения ведения общего хозяйства</h3> " . $question_credits__obligations_rest;
	}
	if ($question_credits__obligations_payer != '-') {
		$message .=
			"<br><br><h3>Кто является плательщиком кредита?</h3> " . $question_credits__obligations_payer;
	}
	if ($question_credits__property_division != '-') {
		$message .=
			"<br><br><h3>Опишите как вы видите раздел имущества, кому что должно остаться в каких долях, как должны быть распределены совместные обязательства?</h3> " . $question_credits__property_division;
	}
